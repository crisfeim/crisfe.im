...
Deno.test("iterates N times if condition is never fullfilled", async () => {
  ...
  const result = await iterator.iterate(maxIterations, action, neverFullfilledCondition)
...
Deno.test("Iterates until condition is fullfilled", async () => {
  ...
  const result = await iterator.iterate(maxIterations, action, breakCondition)
